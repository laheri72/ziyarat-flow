...
export default function Dashboard() {
  ...
  // ⚡ Bolt: Memoize filtered and grouped assignments to prevent unnecessary re-evaluations
  // on every render, especially when the search query or assignments list changes.
  // We also hoist the `searchQuery.toLowerCase()` conversion outside the filter callback
  // to avoid redundant string operations in the loop.
  ...
  const groupedEventTags = useMemo(() => {
    // ⚡ Bolt: Object.keys returns a new array so we can sort it directly, but we useMemo to avoid re-sorting
